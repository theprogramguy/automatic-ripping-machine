...
#"/mnt/media/ARM/raw//THE_MATRIX_20170203_145850" "THE_MATRIX" "false" "unknown" 20170203_145850

#/mnt/media/ARM/raw/*FOLDERNAME*
SRC=$1
#Either DISC_LABEL, or Movie Name (release year)
LABEL=$2
#true or false
HAS_NICE_TITLE=$3
#"movie",tv? || "unknown" -- havent seen tv type yet in actual data
VIDEO_TYPE=$4
# YYYYMMDD_HHMMSS ie 20170203_145850
TIMESTAMP=$5
#"BLURAY" or "DVD" || "ERROR"
MEDIA_TYPE=$6 
#/mnt/media/ARM/Media/Unidentified/*FOLDERNAME*
DEST=$7
#/opt/arm/logs/DISC_ID.LOG
THE_LOG=$8
...
#this should be moved into the video_rip section, this file should only be called when files are in a state that can be proccessed in the background, immediate conversion needs to happen when disc is still in drive and can not be offset till later
	elif [ "$MAINFEATURE" = true ] && [ "$ID_CDROM_MEDIA_DVD" = "1" ]; then
		echo "moved to video_rip"
	else
...
if [ "$EMBY_REFRESH" = true ]; then embyrefresh; fi
